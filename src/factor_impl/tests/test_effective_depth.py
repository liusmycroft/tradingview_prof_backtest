...
@pytest.fixture
def factor():
    return EffectiveDepthFactor()


class TestEffectiveDepthMetadata:
    def test_name(self, factor):
        assert factor.name == "EFFECTIVE_DEPTH"

    def test_category(self, factor):
        assert factor.category == "高频流动性"

    def test_repr(self, factor):
        assert "EFFECTIVE_DEPTH" in repr(factor)

    def test_get_metadata(self, factor):
        meta = factor.get_metadata()
        assert meta["name"] == "EFFECTIVE_DEPTH"
        assert meta["category"] == "高频流动性"


class TestEffectiveDepthHandCalculated:
    def test_constant_input(self, factor):
        dates = pd.date_range("2024-01-01", periods=20, freq="D")
        data = pd.DataFrame(300.0, index=dates, columns=["A"])
        result = factor.compute(daily_effective_depth=data, T=20)
        np.testing.assert_array_almost_equal(result["A"].values, 300.0)

    def test_ema_manual_T3(self, factor):
        dates = pd.date_range("2024-01-01", periods=4, freq="D")
        data = pd.DataFrame([10.0, 20.0, 30.0, 40.0], index=dates, columns=["A"])
        result = factor.compute(daily_effective_depth=data, T=3)
...
class TestEffectiveDepthOutputShape:
