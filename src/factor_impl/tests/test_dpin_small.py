...
@pytest.fixture
def factor():
    return DpinSmallFactor()


class TestDpinSmallMetadata:
    def test_name(self, factor):
        assert factor.name == "DPIN_SMALL"

    def test_category(self, factor):
        assert factor.category == "高频资金流"

    def test_repr(self, factor):
        assert "DPIN_SMALL" in repr(factor)

    def test_get_metadata(self, factor):
        meta = factor.get_metadata()
        assert meta["name"] == "DPIN_SMALL"
        assert meta["category"] == "高频资金流"


class TestDpinSmallHandCalculated:
    """手算验证 rolling mean。"""

    def test_constant_input(self, factor):
        """常数 0.25, T=3 => rolling mean = 0.25。"""
        dates = pd.date_range("2024-01-01", periods=5, freq="D")
        stocks = ["A"]
        daily = pd.DataFrame(0.25, index=dates, columns=stocks)

        result = factor.compute(daily_dpin_small_mean=daily, T=3)

        for val in result["A"].values:
            assert val == pytest.approx(0.25, rel=1e-10)
    ...
    def test_two_stocks_independent(self, factor):
        ...
        stocks = ["A", "B"]
        daily = pd.DataFrame(
            {"A": [0.15] * 5, "B": [0.45] * 5}, index=dates
        )

        result = factor.compute(daily_dpin_small_mean=daily, T=3)

        np.testing.assert_array_almost_equal(result["A"].values, 0.15)
        np.testing.assert_array_almost_equal(result["B"].values, 0.45)

    def test_dpin_bounded_0_1(self, factor):
        """DPIN_SMALL 值在 [0,1] 范围内时, 均值也应在 [0,1]。"""
        dates = pd.date_range("2024-01-01", periods=10, freq="D")
        stocks = ["A"]
        np.random.seed(42)
        daily = pd.DataFrame(
            np.random.uniform(0, 1, (10, 1)), index=dates, columns=stocks
        )

        result = factor.compute(daily_dpin_small_mean=daily, T=5)
        assert (result.values >= 0).all()
        assert (result.values <= 1).all()


class TestDpinSmallEdgeCases:
    def test_single_value(self, factor):
        dates = pd.date_range("2024-01-01", periods=1, freq="D")
        stocks = ["A"]
        daily = pd.DataFrame([0.33], index=dates, columns=stocks)

        result = factor.compute(daily_dpin_small_mean=daily, T=20)
...
class TestDpinSmallOutputShape:
