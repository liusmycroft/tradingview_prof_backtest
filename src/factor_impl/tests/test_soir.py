...
class TestSOIRMetadata:
    def test_name(self, factor):
        assert factor.name == "SOIR"

    def test_category(self, factor):
        assert factor.category == "高频流动性"

    def test_repr(self, factor):
        assert "SOIR" in repr(factor)

    def test_get_metadata(self, factor):
        meta = factor.get_metadata()
        assert meta["name"] == "SOIR"
        assert meta["category"] == "高频流动性"


class TestSOIRHandCalculated:
    ...
    def test_constant_input(self, factor):
        ...
        stocks = ["A"]
        daily_soir = pd.DataFrame(0.3, index=dates, columns=stocks)

        result = factor.compute(daily_soir=daily_soir, T=20)

        valid = result.dropna()
        np.testing.assert_array_almost_equal(valid["A"].values, 0.3)
    ...
    def test_ema_weights_recent_more(self, factor):
        ...
        stocks = ["A"]
        values = [-0.5] * 20 + [0.5] * 5
        daily_soir = pd.DataFrame(values, index=dates, columns=stocks)

        result = factor.compute(daily_soir=daily_soir, T=20)

        last_val = result.iloc[-1, 0]
        assert -0.5 < last_val < 0.5
    ...
    def test_two_stocks_independent(self, factor):
        ...
        daily_soir = pd.DataFrame(
            {"A": [0.1] * 25, "B": [-0.2] * 25}, index=dates
        )

        result = factor.compute(daily_soir=daily_soir, T=20)

        valid = result.dropna()
        np.testing.assert_array_almost_equal(valid["A"].values, 0.1)
    ...
    def test_ema_manual_T3(self, factor):
        ...
        result = factor.compute(daily_soir=daily_soir, T=3)
...
class TestSOIREdgeCases:
    def test_nan_in_input(self, factor):
        """输入含 NaN 时不应抛异常。"""
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        stocks = ["A"]
        values = np.ones(25) * 0.3
        values[10] = np.nan
        daily_soir = pd.DataFrame(values, index=dates, columns=stocks)
    ...
    def test_all_nan(self, factor):
        ...
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        stocks = ["A"]
        daily_soir = pd.DataFrame(np.nan, index=dates, columns=stocks)
    ...
    def test_zero_input(self, factor):
        ...
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        stocks = ["A"]
        daily_soir = pd.DataFrame(0.0, index=dates, columns=stocks)
...
class TestSOIROutputShape:
    def test_output_shape_matches_input(self, factor):
        ...
        assert result.shape == daily_soir.shape
        assert list(result.columns) == list(daily_soir.columns)
        assert list(result.index) == list(daily_soir.index)

    def test_output_is_dataframe(self, factor):
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        stocks = ["A"]
        daily_soir = pd.DataFrame([0.1] * 25, index=dates, columns=stocks)
    ...
    def test_first_T_minus_1_rows_are_nan(self, factor):
        ...
        result = factor.compute(daily_soir=daily_soir, T=T)
