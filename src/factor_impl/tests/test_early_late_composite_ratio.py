...
@pytest.fixture
def factor():
    return EarlyLateCompositeRatioFactor()


class TestEarlyLateCompositeRatioMetadata:
    def test_name(self, factor):
        assert factor.name == "EARLY_LATE_COMPOSITE_RATIO"

    def test_category(self, factor):
        assert factor.category == "高频资金流"

    def test_repr(self, factor):
        assert "EARLY_LATE_COMPOSITE_RATIO" in repr(factor)

    def test_get_metadata(self, factor):
        meta = factor.get_metadata()
        assert meta["name"] == "EARLY_LATE_COMPOSITE_RATIO"
        assert meta["category"] == "高频资金流"


class TestEarlyLateCompositeRatioHandCalculated:
    def test_constant_input(self, factor):
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        data = pd.DataFrame(0.3, index=dates, columns=["A"])
        result = factor.compute(daily_composite_ratio=data, T=20)
    ...
    def test_rolling_mean_T3(self, factor):
        ...
        result = factor.compute(daily_composite_ratio=data, T=3)
    ...
    def test_two_stocks_independent(self, factor):
        dates = pd.date_range("2024-01-01", periods=5, freq="D")
        data = pd.DataFrame({"A": [0.1] * 5, "B": [0.5] * 5}, index=dates)
        result = factor.compute(daily_composite_ratio=data, T=3)
        assert result.iloc[2, 0] == pytest.approx(0.1)
        assert result.iloc[2, 1] == pytest.approx(0.5)


class TestEarlyLateCompositeRatioEdgeCases:
    ...
    def test_all_nan(self, factor):
        ...
        result = factor.compute(daily_composite_ratio=data, T=5)
...
class TestEarlyLateCompositeRatioOutputShape:
    ...
    def test_first_T_minus_1_rows_are_nan(self, factor):
        dates = pd.date_range("2024-01-01", periods=25, freq="D")
        data = pd.DataFrame(
            np.random.uniform(-0.5, 0.5, (25, 2)), index=dates, columns=["A", "B"]
        )
        result = factor.compute(daily_composite_ratio=data, T=20)
