...
@pytest.fixture
def factor():
    return InformedBuyRatioFactor()


class TestInformedBuyRatioMetadata:
    def test_name(self, factor):
        assert factor.name == "INFORMED_BUY_RATIO"

    def test_category(self, factor):
        assert factor.category == "高频资金流"

    def test_repr(self, factor):
        assert "INFORMED_BUY_RATIO" in repr(factor)
        assert "InformedBuyRatioFactor" in repr(factor)

    def test_get_metadata(self, factor):
        meta = factor.get_metadata()
        assert meta["name"] == "INFORMED_BUY_RATIO"
        assert meta["category"] == "高频资金流"
        assert "知情" in meta["description"]


class TestInformedBuyRatioHandCalculated:
    """用手算数据验证 ratio = informed_buy / total, 再取滚动均值。"""

    def test_T3_single_stock(self, factor):
        """T=3, 单只股票, 手动计算验证。

        informed_buy = [100, 200, 300]
        ...
        result = factor.compute(informed_buy_amount=informed, total_amount=total, T=3)
    ...
    def test_T2_two_stocks(self, factor):
        ...
        Stock A: informed=[100, 300], total=[1000, 1000]
                 ratio=[0.1, 0.3], rolling mean(T=2) at t=1: 0.2
        Stock B: informed=[500, 100], total=[1000, 1000]
                 ratio=[0.5, 0.1], rolling mean(T=2) at t=1: 0.3
        """
        ...
        result = factor.compute(informed_buy_amount=informed, total_amount=total, T=2)
...
class TestInformedBuyRatioEdgeCases:
    ...
    def test_nan_in_total(self, factor):
        """total_amount 中含 NaN 时, 结果不应抛异常。"""
        dates = pd.date_range("2024-01-01", periods=3, freq="D")
        stocks = ["A"]

        informed = pd.DataFrame([100.0, 200.0, 300.0], index=dates, columns=stocks)
        total = pd.DataFrame([1000.0, np.nan, 1000.0], index=dates, columns=stocks)
    ...
    def test_zero_informed_buy(self, factor):
        """知情买入为 0 时, ratio=0, 均值=0。"""
        dates = pd.date_range("2024-01-01", periods=3, freq="D")
        stocks = ["A"]
...
class TestInformedBuyRatioOutputShape:
    def test_output_shape_matches_input(self, factor):
        ...
        result = factor.compute(informed_buy_amount=informed, total_amount=total, T=20)
    ...
    def test_first_T_minus_1_rows_are_nan(self, factor):
        ...
        result = factor.compute(informed_buy_amount=informed, total_amount=total, T=T)
